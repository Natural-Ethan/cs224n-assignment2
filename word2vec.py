...
def naiveSoftmaxLossAndGradient(
    ...
    product = outsideVectors.dot(centerWordVec)
    result = np.reshape(softmax(product), (-1, 1))
    loss = - np.log(result[outsideWordIdx, 0])
    
    # compute the gradients
    temp = np.zeros(shape=result.shape)
    temp[outsideWordIdx, 0] = 1

    gradCenterVec = - (outsideVectors[outsideWordIdx] - np.sum(result * outsideVectors, axis=0)) 
    gradOutsideVecs = - centerWordVec * (temp - result)
...
def negSamplingLossAndGradient(
    ...
    positive = sigmoid(product[indices[0]])
    negative = np.reshape(sigmoid( - product[indices[1:]]), (-1, 1))

    loss = - np.log(positive) - np.sum(np.log(negative))

    # compute the gradients
    gradCenterVec = outsideVectors[indices[0]] * (positive - 1) - np.sum(outsideVectors[indices[1:]] * (negative - 1), axis=0)

    gradOutsideVecs = np.zeros(shape=outsideVectors.shape)
    for i in range(1,len(indices)):
        gradOutsideVecs[indices[i]] += centerWordVec * (1 - negative[i - 1])
       
    gradOutsideVecs[indices[0]] += centerWordVec * (positive - 1)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centerWordVectors -- center word vector"s (as rows) is in shape 
    ...
    for word in outsideWords:
        ...
        loss_, gradC_, gradO_ = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += loss_
        gradCenterVecs[centerWordIdx] += gradC_
        gradOutsideVectors += gradO_
